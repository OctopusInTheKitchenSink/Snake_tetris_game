...
bool time_passed(int level) {
  bool flag = false;
  static struct timespec tm = {0, 0};
  struct timespec now;
  clock_gettime(CLOCK_MONOTONIC, &now);
  if (tm.tv_nsec == 0 && tm.tv_sec == 0) {
    tm = now;
  } else {
    long seconds = now.tv_sec - tm.tv_sec;
    long nanoseconds = now.tv_nsec - tm.tv_nsec;
    double elapsed_time_ms = (seconds * 1000.0) + (nanoseconds / 1000000.0);
    double time_delay = (double)START_TIMEOUT / level * 1.4;
...
void game_active(GameInfo_t *game, Figure_position *figure) {
  bool check_flag =
      (game->pause != game_pause && game->pause != ready_to_start &&
